...
const AllRooms = () => {
    ...
    const handlerSortChange = (sortOption) => {
    ...
    // Filter and sort rooms based on the seleced filters and sort option
    const filteredRooms = useMemo(() => {
        return rooms.filter(room => matchesRoomType(room) && matchesPriceRange(room) && filterDestination(room)).sort(sortRoom);
                        ...
                        <div className='md:w-1/2 flex flex-col gap-2'>
                            ...
                            <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                                {room.amenities.map((item, i) => (
                                    <div key={item._id} className='flex items-center gap-2 px-3 py-2 rounded-lg bg-[#F5F5FF]/70'>
                    ...
                    <div className='px-5 pt-5'>
                        ...
                        {roomTypes.map((room, i) => (
                            <CheckBox key={i} label={room}
                                ...
                                onChange={(checked) => handlerFilterChange(checked, room, 'roomTypes')}
                            />
                        ))}
                    </div>
                    <div className='px-5 pt-5'>
                        ...
                        {priceRanges.map((range, i) => (
                            <CheckBox key={i} label={`$${currency} ${range}`}
                                selected={selectedFilters.priceRange.includes(range)}
                                onChange={(checked) => handlerFilterChange(checked, range, 'priceRange')}
                            />
                        ))}
                    </div>
                    <div className='px-5 pt-5 pb-7'>
                        ...
                        {sortOptions.map((option, i) => (
                            <RadioButton key={i} label={option}
                                ...
                                onChange={() => handlerSortChange(option)}
                            />
                        ))}
                    </div>
                </div>
