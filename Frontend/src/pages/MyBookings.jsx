...
const MyBookings = () => {
  ...
  const {axios, getToken, user, baseURL} = useAppContext()
  const [bookings, setBookings] = useState([])

  const fetchUserBookings = async () => {
    try {
      const {data} = await axios.get(`${baseURL}/api/booking/user`, {headers: {Authorization: `Bearer ${getToken()}`}})
      if(data.success) {
  ...
  const handlePayment = async (bookingId) => {
    try {
      const {data} = await axios.post(`${baseURL}/api/bookings/stripe-payment`, {bookingId}, {headers: {Authorization: `Bearer ${getToken()}`}})
      if(data.success) {
