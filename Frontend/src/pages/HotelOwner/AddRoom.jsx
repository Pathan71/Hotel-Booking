...
const AddRoom = () => {
  ...
  const { axios, getToken, baseURL } = useAppContext()
  ...
  const onSubitHandler = async (e) => {
    ...
    if (!inputs.roomType || !inputs.pricePerNight || !inputs.amenities || !Object.values(images).some(image => image)) {
      toast.error("Please fill in all the deatails")
      return;
    }
    setLoading(true)
    try {
      ...
      // Converting Amenities to Arrya & keeping only enabled Amenities
      ...
      const { data } = await axios.post(`${baseURL}/api/rooms/`, formData, { headers: { Authorization: `Bearer ${await getToken()}` } })
    ...
    <form onSubmit={onSubitHandler}>
      ...
      <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
        {Object.keys(inputs.amenities).map((amenity, i) => (
          <div key={i}>
            <input type="checkbox" id={`amenities${i + 1}`}
