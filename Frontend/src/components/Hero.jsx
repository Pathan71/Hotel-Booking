...
const Hero = () => {
    ...
    const { navigate, getToken, axios, baseURL, setSearchedCities } = useAppContext()
    ...
    const onSearch = async (e) => {
        ...
        await axios.post(`${baseURL}/api/user/store-recent-search`, { recentSearchedCity: destination }, { headers: { Authorization: `Bearer ${await getToken()}` } })

        // add destination to serachedCities max 3 recent serached cities
            ...
            <form onSubmit={onSearch} className='bg-white text-gray-500 rounded-lg px-6 py-4 mt-8 flex flex-col md:flex-row max-md:items-start gap-4 max-md:mx-auto'>
                ...
                <div>
                    ...
                    <datalist id='destinations'>
                        {cities.map((city, i) => (
                            <option value={city} key={i}></option>
                        ))}
                    </datalist>
                </div>
